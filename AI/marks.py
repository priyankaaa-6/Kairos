...
def viz(marks, q_time):
    ...
    with PdfPages('report_card.pdf') as pdf:
        ...
        # Handle NaN values in sizes and replace them with 0 if any
        sizes = [correct_count, wrong_count]
        
        # Check if both sizes are zero and avoid plotting in that case
        if np.isnan(sizes[0]) or np.isnan(sizes[1]):
            sizes = [0, 0]

        # Explicitly replace NaN values (if any) in sizes list with zero
        sizes = [0 if np.isnan(i) else i for i in sizes]

        # Check if sizes is valid and contains non-zero values
        if sizes[0] == 0 and sizes[1] == 0:
            print("Warning: No data to plot for the pie chart.")
            return  # Avoid plotting if both values are zero
